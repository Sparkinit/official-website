...
import { darkMode, intensity } from "@/stores";
...
const PrismaticBurst = ({
  ...
  const smoothIntensityRef = useRef<number>(propIntensity ?? 1);
  const targetIntensityRef = useRef<number>(propIntensity ?? 1);

  const $darkMode = useStore(darkMode);
  const $intensity = useStore(intensity);
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      const intensityAlpha = 1 - Math.exp(-dt / 0.2);
      smoothIntensityRef.current +=
        (targetIntensityRef.current - smoothIntensityRef.current) *
        intensityAlpha;
      program.uniforms.uIntensity.value = smoothIntensityRef.current;
  ...
  useEffect(() => {
    ...
    let finalIntensity = propIntensity ?? $intensity ?? 2;
    if (adaptToDarkMode) {
      finalIntensity = $darkMode ? finalIntensity * 0.7 : finalIntensity;
    }

    targetIntensityRef.current = finalIntensity;
  ...
  const backgroundColor = adaptToDarkMode
    ? $darkMode
      ? darkModeBackground
      : lightModeBackground
    : "transparent";
  ...
  return (
    <div
      className="ease relative h-full w-full overflow-hidden transition-colors duration-300"
